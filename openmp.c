...
void forcaBrutaOPM(){
	...
	for(i = 1; i < numerothreads; i++){
        ...
        //Se há possível ocorrência entre duas threads
		if(verificarFinalThread(dicionario[inicioThreads[i]], dicionario[fimThreads[i-1]])){
			inicioThreads[i] -= tamanhoPadrao;
		}
		if(i == numerothreads - 1){ //A ulima thread fica responsável por tratar o resto da divisão.
			fimThreads[i] += resto;
		}
	}
	...
	for(i = 0; i < numerothreads; i++){     //Chamando a função para cada thread.
